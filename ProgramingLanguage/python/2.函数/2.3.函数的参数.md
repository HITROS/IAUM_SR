...
125
```
修改后的`power(x, n)`函数有两个参数：x和n，这两个参数都是位置参数，调用函数时，传入的两个值按照位置顺序依次赋给参数x和n。
### 2. 默认参数
...
14
```
这种写法当然是可行的，问题是太繁琐，所以Python~~允许你在list或tuple前面加一个*号~~，把list或tuple的元素变成可变参数传进去：
```py
>>> nums = [1, 2, 3]
>>> calc(*nums)
14
...
使用`*args`和`**kw`是Python的**习惯写法**，当然也可以用其他参数名，但最好使用习惯用法。
